...
def main():
    ...
    parser.add_argument('--select_every', default=20, type=int, help='select subset every x epoch')
    ...
    # for accelerate training, this is not needed
    ...
    parser.add_argument('--in_dim', type=int, default=768, help='the feature dimension')
    parser.add_argument('--num_classes', type=int, default=10, help='the class number')
    parser.add_argument('--window_ratio',  type=float, default=0, help='noise window ratio')
    ...
    # use wandb
    watermark = "{}_{}_{}_lr_{}_window_{}".format(args.dataset, args.fraction, args.model, args.lr, args.window_ratio)
    wandb.init(project="tl-ratio-bench", name=watermark)
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        dst_train, dst_test, dst_val = datasets.__dict__[args.dataset](args.data_path)
        ...
        for model in models:
            ...
            print("feature dim: ", in_dim, " class: ", num_classes)
            network = nets.__dict__[model](in_dim, num_classes).to(args.device)
            ...
            warm_ratio = args.warm_epoch / args.epochs
            n_interation  = math.ceil(len(dst_train) * args.fraction / args.train_batch) * args.epochs * (1- warm_ratio) + round(len(dst_train) / args.train_batch) * args.epochs * warm_ratio
            ...
            scheduler.last_epoch = (start_epoch - 1) * n_sub_batch
            ...
            for epoch in range(start_epoch, args.epochs):
                ...
                if (args.warm_epoch != 0):
                    if epoch == 0:
                        ...
                        method = methods.__dict__['AdaFull'](dst_train, network, args, args.fraction, args.seed, **selection_args)
                        subset = method.select()
                
                
                if (epoch >= args.warm_epoch and (epoch-args.warm_epoch) % args.select_every == 0):
                    ...
                    if "subset" in checkpoint.keys():
                        ...
                        if "AdaUniformBin" in args.selection:
                            print("selection method: RandomBin.")
                            method = methods.__dict__['AdaUniformBin'](dst_train, network, args, args.fraction, args.seed, **selection_args)
                            subset = method.select()
                        else:
                            print("selection method: Random.")
                            method = methods.__dict__['AdaUniform'](dst_train, network, args, args.fraction, args.seed, **selection_args)
                            subset = method.select()
                    else:
                    ...
                    print("At epoch{}, select {} samples, take {} s.".format(epoch, len(subset["indices"]), select_time))
                ...
                epoch_train_time = train(train_loader, network, criterion, optimizer, scheduler, epoch, args, rec, if_weighted=if_weighted)
                ...
                if args.test_interval > 0 and (epoch + 1) % args.test_interval == 0:
                    prec1, epoch_test_time = test(test_loader, network, criterion, epoch, args, rec)
