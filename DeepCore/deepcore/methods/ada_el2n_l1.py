...
class AdaEL2NL1(AdaCoresetMethod):
    ...
    def select(self, **kwargs):
        ...
        # np.savez('./index/all_data_{}_{}_{}_{}.npz'.format(self.args.dataset, self.args.selection, self.args.fraction, datetime.datetime.now().strftime("%d")), norm_mean=self.norm_mean, train_indx=self.train_indx)
        if not self.balance:
            # not the top, use a window
            print("use a window")
            window_start = round(self.n_train * self.args.window_ratio)
            # window_start = 0
            top_examples = self.train_indx[np.argsort(self.norm_mean)][::-1][window_start: window_start + self.coreset_size]
        else:
        ...
        # np.savez('./index/selected_data_{}_{}_{}_{}.npz'.format(self.args.dataset, self.args.selection, self.args.fraction, datetime.datetime.now().strftime("%d")), train_indx=top_examples)
