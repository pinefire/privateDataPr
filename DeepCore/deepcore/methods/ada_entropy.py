...
class AdaEntropy(AdaCoresetMethod):
    ...
    def finish_run(self):
        ...
        for i, (input, targets) in enumerate(batch_loader):
            ...
            with torch.no_grad():
                ...
                self.norm_matrix[i * self.args.selection_batch:min((i + 1) * self.args.selection_batch, sample_num),
                self.cur_repeat] = self.criterion(outputs, targets.to(self.args.device)).sum()
    ...
    def select(self, **kwargs):
        ...
        print("called Entropy, with {} ensemble and {} epochs, {} all samples".format(self.repeat, self.epochs, len(self.dst_train)))
